    ...
    methods: {
        ...
        toggleFilter() {
            if (!this.filterActive) {
                // Simpan daftar kegiatan asli sebelum filter diterapkan
                this.originalKegiatanList = this.kegiatanList.slice();
                this.kegiatanList = this.kegiatanList.filter(kegiatan => kegiatan.status === 'Belum Selesai');
            } else {
                // Kembalikan daftar kegiatan asli setelah filter dihapus
                if (this.originalKegiatanList.length > 0) {
                    this.kegiatanList = this.originalKegiatanList.slice();
                    this.originalKegiatanList = [];
                }
            }
